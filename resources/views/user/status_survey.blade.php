...
<div class="container">
  <div class="card">
    <div class="card-body">
        ...
        <table class="table table-hover datatab">
          ...
          <tbody>
            ...
            <tr style="text-align: center;">
              ...
              <!-- button modal -->
              <td><button id="btn-f" type="button" class="btn btn-outline-primary btn-detail" data-bs-toggle="modal" data-bs-target="#detail-survey" data-id="{{ $data1->id_surat_survei }}">Detail</button></td>
              @if ($data1->status != 'Disetujui' && $data1->status != 'Ditolak' && $data1->status != 'Sedang Diproses')
              <td>
                <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                  <form action="{{ route('survey.destroy', $data1->id_surat_survei) }}" method="POST" style="display: inline;">
                    @method('DELETE')
                    @csrf
                  ...
                  <form>
                    <button id="btn-f" type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#edit-survey-{{ $data1->id_surat_survei }}" data-id="{{ $data1->id_surat_survei }}">Edit</button>
                  </form>
                </div>
              </td>
              @else <td>
      ...
      <div class="modal fade" id="edit-survey-{{ $data_edit->id_surat_survei }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
          <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
              <form action="{{ route('survey.update', $data_edit->id_surat_survei) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="container-fluid">
                  <div class="row">
